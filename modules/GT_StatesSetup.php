...
class GT_StatesSetup extends APP_GameClass
{
  ...
  function stPrepareRound($game, $players)
  {
    ...
    $game->DbQuery(
      'UPDATE player SET still_flying=1, turn_order=NULL, player_position=NULL, nb_crew=NULL, ' .
    ...
    // Prepare ships
    $game->DbQuery(
      'UPDATE component SET component_player=NULL, component_x=NULL, ' .
    ...
    foreach ($players as $player_id => $player) {
      ...
      $game->DbQuery(
        'UPDATE component SET component_x=7, component_y=7, ' . "component_player=$player_id WHERE component_id=$id"
      );
      // Expansions: need to be changed for expansions' ship classes
      // that don't have a starting component at the beginning
      $startingTiles[$id] = ['id' => $id, 'x' => 7, 'y' => 7, 'player' => $player_id, 'o' => 0];
    ...
    $game->DbQuery(
      'UPDATE component SET component_player=0 WHERE component_player IS NULL ' .
    ...
    $tilesLeft = $game->getUniqueValueFromDB(
    ...
    ////// Notify all
    $game->notifyAllPlayers('newRound', '', [
      'shipClass' => $shipClass,
      'tilesLeft' => $tilesLeft,
  ...
  function stPrepareFlight($game, $players)
  {
    ...
    foreach ($players as $plId => $player) {
      ...
      $game->DbQuery("UPDATE player SET player_position=$playerPos " . "WHERE player_id=$plId");
      $game->notifyAllPlayers('placeShipMarker', '', [
        'player_id' => $plId,
        'plPos' => $playerPos,
        ...
        foreach ($plBoard_x as $tile) {
          ...
          switch ($tileType) {
            ...
            case 'crew': // Expansions: and case 'luxury':
              ...
              $tilesWithOverlay[] = ['id' => $tile['id'], 'x' => $tile['x'], 'y' => $tile['y']];
              ...
              if (array_key_exists($tile['id'], $game->start_tiles)) {
                ...
                foreach ($brd->getConnectedTiles($tile) as $adjTile) {
                  switch ($game->getTileType($adjTile['id'])) {
                    case 'brown':
                      if (!$brownPresent) {
                        // Because we don't want to count twice
      ...
      // Database update:
      $sql =
        'INSERT INTO content (player_id, tile_id, square_x, square_y, content_type, ' .
        'content_subtype, place, capacity) VALUES ' .
        implode(',', $sqlImplode);
      $game->DbQuery($sql);
      ...
      $game->notifyAllPlayers('updateShipContent', '', [
        'player_id' => $plId,
  ...
  function cardsIntoPile($game, $cdRound, $nbCards)
  {
    for ($i = 1; $i <= 4; $i++) {
      // TODO replace RAND
      $game->DbQuery(
        "UPDATE card SET card_pile=$i WHERE card_round=$cdRound " .
