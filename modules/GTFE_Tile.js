class GTFE_Tile {
  ...
  constructor(game, id) {
    ...
    if (typeof id == 'string' && id.startsWith('tile_')) id = id.split('_')[1];
    this.game = game;
    this.id = id;
  ...
  getType(game, id) {
    return game.tiles[id]['type'];
  }
  getHold(game, id) {
    return game.tiles[id]['hold'];
  }
  ...
  getPlaceClass(place) {
    ...
    if (place) {
      if (place > this.hold) this.game.throw_bug_report('Place cannot be greater than hold: ' + place + ', ' + this.id);
    ...
    for (let i = 1; i <= 3; i++) {
      if (i > this.hold) break;
      let tgt_class = 'p' + i + 'on' + this.hold;
      if (curNodes.filter((n) => n.classList.contains(tgt_class)).length == 0) return tgt_class;
    }
  ...
  placeContent(cont, slideFromId) {
    ...
    if (cont.tile_id != this.id) game.throw_bug_report('Wrong content for tile (' + cont.tile_id + ',' + this.id + ')');
  ...
  slideContent(contDiv, place, duration, delay) {
    ...
    dojo.connect(
      anim,
      'onEnd',
      dojo.hitch(this, function (contDiv) {
  ...
  loseContent(cont, delay, toCard) {
    ...
    if (toCard === undefined) toCard = cont.toCard;
    console.log('after', toCard);

    let divId = '';
    if (typeof cont.id != 'string' || !cont.id.startsWith('content_')) divId = 'content_' + cont.id;
    else divId = cont.id;
    ...
    if (toCard) {
      ...
      var anim = dojo.fx.combine([
        dojo.fx.slideTo({ node: divId, left: 100, top: 400, units: 'px', duration: 1700, delay: delay }),
  ...
  _attachToSquare(contDiv) {
    ...
    if (typeof contDiv == 'string') contDiv = $(contDiv);
