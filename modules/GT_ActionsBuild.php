...
class GT_ActionsBuild extends APP_GameClass
{
  ...
  function pickTile()
  {
    ...
    $game->DbQuery(
      "UPDATE component SET component_player=$plId " . 'WHERE component_player IS NULL ORDER BY RAND() LIMIT 1'
    );
    ...
    $game->notifyPlayer($plId, 'pickedTilePl', '', ['pickedTile' => $pickedTile]);
    $game->notifyAllPlayers('pickedTile', '', []);
  }
  ...
  function pickRevealed($tile_id)
  {
    ...
    $location = $game->getUniqueValueFromDB('SELECT component_player FROM component ' . "WHERE component_id=$tile_id");
    if ($location !== '-1') {
      $game->user_exception('This component has already been taken by someone else');
    }

    $game->DbQuery("UPDATE component SET component_player={$this->plId} " . "WHERE component_id=$tile_id");
    $game->DbQuery("UPDATE revealed_pile SET tile_id=NULL WHERE tile_id=$tile_id");
    $this->resetUndoPossible();

    $game->notifyAllPlayers('pickedRevealed', '', [
      'tile_id' => $tile_id,
      'player_id' => $this->plId,
    ]);
  }
  ...
  function pickAside($tile_id)
  {
    ...
    $pickedTile = $game->getObjectFromDB('SELECT * FROM component ' . "WHERE component_id=$tile_id");
    $x = $pickedTile['component_x'];
    $aside = $pickedTile['aside_discard'];
    if ($pickedTile['component_player'] != $this->plId || $aside !== '1' || ($x != '-1' && $x != '-2')) {
      $game->throw_bug_report('This tile is not in your discard.' . " (pl: {$this->plId} x: $x aside: $aside)");
    }

    $game->DbQuery('UPDATE component SET component_x=NULL, component_y=NULL ' . "WHERE component_id=$tile_id");
    $this->resetUndoPossible();
    $game->notifyAllPlayers('pickedAside', '', [
      'tile_id' => $tile_id,
      'player_id' => $this->plId,
      'discardSquare' => $x,
    ]);
  ...
  function pickLastPlaced($tile_id)
  {
    ...
    $undoTile = $game->getUniqueValueFromDB('SELECT undo_possible FROM player ' . "WHERE player_id=$plId");
    if ($undoTile !== $tile_id) {
      // both are strings
      $game->throw_bug_report("You can't take back this tile. " . " (pl: $plId tile id: $tile_id undo: $undoTile)");
    }
    ...
    $pickedTile = $game->getObjectFromDB(
      'SELECT component_x x, component_y y, ' . "aside_discard aside FROM component WHERE component_id=$tile_id"
    );
    // another sanity check?
    if (!($pickedTile['x'] > 0) || !($pickedTile['y'] > 0)) {
      $game->throw_bug_report('PickLastPlaced: tile coords are ' . $pickedTile['x'] . ' and ' . $pickedTile['y']);
    }

    $game->DbQuery('UPDATE component SET component_x=NULL, component_y=NULL ' . "WHERE component_id=$tile_id");
    $this->resetUndoPossible('lastPlaced');
    $game->notifyAllPlayers('pickedLastPlaced', '', [
      'tile_id' => $tile_id,
      'x' => $pickedTile['x'],
      'y' => $pickedTile['y'],
  ...
  function dropTile($tile_id)
  {
    ...
    $location = $this->game->getObjectFromDB(
      'SELECT component_player player, component_x x, ' .
  ...
  function placeTile($component_id, $x, $y, $o)
  {
    ...
    $cards = null;
    $allTiles = $game->getCollectionFromDB(
      'SELECT component_id id, component_x x, component_y y, ' .
    ...
    $tileToCheck = ['x' => $x, 'y' => $y, 'id' => $component_id, 'o' => $o];
    ...
    $game->DbQuery("UPDATE player SET undo_possible=$component_id " . "WHERE player_id=$player_id");
  ...
  function discardTile($component_id, $x, $y, $o)
  {
    ...
    if ($o !== '0') {
      // Do we really need to raise an exception for that? We could
      // log it and set $o to 0.
      $game->throw_bug_report("tile orient is $o, should be 0.");
    }

    $setAsideTiles = $game->getCollectionFromDB(
      'SELECT component_id id, component_x x, component_y y, ' .
    ...
    $sql =
      "UPDATE component SET component_x=$x, component_y=NULL, " .
  ...
  function flipTimer($timerPlace, $player_name)
  {
    ...
    $turnOrder = $game->getUniqueValueFromDB('SELECT turn_order FROM player ' . "WHERE player_id=$player_id");
    if ($timerPlace == 1 && $turnOrder === null) {
      $game->throw_bug_report(
        "Flip timer: you can't flip the timer on " . 'the last space when you are still building your ship. '
      );
    }
    ...
    // Notify players
    $game->notifyAllPlayers('timerFlipped', clienttranslate('${player_name} has flipped the timer.'), [
      'player_id' => $player_id,
  ...
  function timeFinished()
  {
    ...
    $tilesInHand = $game->getCollectionFromDB(
    ...
    foreach ($tilesInHand as $tileId => $tile) {
      ...
      if ($tile['aside'] == 1) {
        ...
        $occupiedSquare = $game->getUniqueValueFromDB(
          'SELECT component_x x FROM ' . "component WHERE component_player=$plId AND aside_discard=1 AND component_x<0"
        );

        // No tile in discard, so the tile in hand goes on the 1st square
        if (!$occupiedSquare) {
          $squareToDiscardTo = -1;
        } elseif ($occupiedSquare == -1) {
          $squareToDiscardTo = -2;
        } elseif ($occupiedSquare == -2) {
          $squareToDiscardTo = -1;
        } else {
          $game->throw_bug_report_dump("Bad value for \$occupiedSquare: ", $occupiedSquare);
        }

        $game->DbQuery(
          "UPDATE component SET component_x=$squareToDiscardTo, " . "component_orientation=0 WHERE component_id=$tileId"
        );
        $game->notifyAllPlayers('placedTile', '', [
          'player_id' => $plId,
          'component_id' => $tileId,
  ...
  function finishShip($orderTile)
  {
    ...
    $count = $game->getUniqueValueFromDB('SELECT COUNT(player_id) FROM player ' . "WHERE turn_order=$orderTile");
    if ($count !== '0') {
    ...
    $game->DbQuery("UPDATE player SET turn_order=$orderTile, " . "undo_possible=NULL WHERE player_id=$player_id");

    $game->DbQuery('UPDATE component SET aside_discard=NULL ' . "WHERE component_x>0 AND component_player=$player_id");

    $game->notifyAllPlayers(
      'finishedShip',
      clienttranslate('${player_name} has finished thier ship!' . ' Order tile: ${orderTile}'),
      ['player_id' => $player_id, 'player_name' => $players[$player_id]['player_name'], 'orderTile' => $orderTile]
    );
  }
  ...
  function crewPlacementDone($alienChoices, $player_name)
  {
    ...
    $contAsk = $game->getCollectionFromDB(
      'SELECT content_id, tile_id, square_x, ' .
    ...
    $game->DbQuery("DELETE FROM content WHERE player_id=$plId AND " . "content_subtype LIKE 'ask_%'");
    // Add aliens and humans
    $sql =
      'INSERT INTO content (player_id, tile_id, square_x, square_y, ' .
      'content_type, content_subtype, place, capacity) ' .
      'VALUES ' .
      implode(',', $sqlImplode);
    $game->DbQuery($sql);
    // Get new content (with auto-incremented content_id) to notify players
    $shipContentUpdate = $game->getCollectionFromDB(
      'SELECT * FROM content ' . 'WHERE tile_id IN (' . implode(',', array_keys($tilesToFill)) . ')'
    );
    ...
    if ($nbAliens == 0) {
      ...
      $notifyText = clienttranslate('${player_name} has chosen one alien: ') . $game->translated[$color] . '.';
    } elseif ($nbAliens == 2) {
      $notifyText = clienttranslate('${player_name} has chosen a brown alien ' . 'and a purple alien.');
    }

    $game->notifyAllPlayers('updateShipContent', $notifyText, [
      // on utilise notif_updateShipContent ou pas ?
      'player_name' => $player_name,
      'player_id' => $plId,
  ...
  function resetUndoPossible($action = '')
  {
    ...
    if ($action !== 'lastPlaced') {
      $lastPlaced = $game->getUniqueValueFromDB('SELECT undo_possible FROM player ' . "WHERE player_id={$this->plId}");
      if ($lastPlaced !== null) {
        $game->DbQuery('UPDATE component SET aside_discard=NULL ' . "WHERE component_id=$lastPlaced");
      }
    }
  ...
  function getTileInHand()
  {
    return $this->game->getUniqueValueFromDB(
      'SELECT * FROM component ' . "WHERE component_player={$this->plId} AND component_x IS NULL"
    );
  }
  ...
  function placeInRevealed($tile_id)
  {
    ...
    $firstAvailSp = $game->getUniqueValueFromDB(
      'SELECT space FROM revealed_pile ' . 'WHERE tile_id IS NULL ORDER BY space LIMIT 1'
    );
    if ($firstAvailSp === null) {
      $game->throw_bug_report('Error: no empty space found in revealed pile ' . "(\$firstAvailSp is null). ");
    }

    // Place dropped tile in revealed pile
    $game->DbQuery(
      'UPDATE component SET component_player=-1, component_orientation=0 ' . "WHERE component_id=$tile_id"
    );
    $game->DbQuery("UPDATE revealed_pile SET tile_id=$tile_id WHERE space=$firstAvailSp");

    $game->notifyAllPlayers('droppedTile', '', [
      'tile_id' => $tile_id,
      'player_id' => $this->plId,
