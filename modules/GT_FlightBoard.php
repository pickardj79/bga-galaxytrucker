...
class GT_FlightBoard extends APP_GameClass
{
  public function __construct($game, $players)
  {
    ...
    $erredPlayers = array_filter($players, function ($i) {
      return array_key_exists('still_flying', $i) && $i['still_flying'] != '1';
    });
  ...
  function addCredits(int $plId, int $credits)
  {
    ...
    $this->game->notifyAllPlayers('addCredits', clienttranslate('${player_name} gains ${credits} galatic credits'), [
      'player_name' => $this->players[$plId]['player_name'],
      'player_id' => $plId,
      'credits' => $credits,
    ]);
  }
  ...
  function moveShip(int $plId, int $nbDays)
  {
    ...
    if ($nbDays === 0) {
      $game->notifyAllPlayers('onlyLogMessage', clienttranslate('${player_name} doesn\'t move'), [
        'player_name' => $plName,
      ]);
      return null;
    } else {
      $trslStr =
        $nbDays > 0
          ? clienttranslate('${player_name} gains ${numDays} flight days')
  ...
  function giveUp(int $plId, $reason)
  {
    ...
    $this->game->notifyAllPlayers('giveUp', clienttranslate('${player_name} will have to give up: ${reason}'), [
      'player_name' => $this->players[$plId]['player_name'],
      'player_id' => $plId,
      'reason' => $reason,
    ]);
