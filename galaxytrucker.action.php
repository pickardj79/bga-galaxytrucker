...
class action_galaxytrucker extends APP_GameAction
{
  ...
  public function contentChoice()
  {
    ...
    $contChoices = $contChoiceRaw === '' ? [] : explode(',', $contChoiceRaw);

    $contType = self::getArg('contentType', AT_alphanum, true);
    switch ($contType) {
      case 'engine':
        $this->game->powerEngines($contChoices);
        break;
      case 'cannon':
        $this->game->powerCannons($contChoices);
        break;
      case 'shield':
        $this->game->powerShields($contChoices);
        break;
      case 'cell':
        $this->game->loseContentChoice('cell', $contChoices);
        break;
      case 'goods':
        $this->game->loseContentChoice('goods', $contChoices);
        break;
      case 'crew':
